...
class MyMainWindow(QMainWindow):
    ...
    def start_modbus(self):
        ...
        if os_path.exists(opt_sentinel_file_name):
            os_remove(opt_sentinel_file_name)
        QThread.msleep(100)
        self.show_back()
        self.window.hide()
        QThread.msleep(100)
    ...
    def check_sensors_ready(self, check_status):
        ...
        if self.sensor_ref_check == 2 and (self.sensor_opt_check == 5 or self.opt_force):
            # print("TERMINATION ------------------------")
            self.thcfgs.set_termination(True)
    ...
    def closeEvent(self, event):
        ...
        if reply == QMessageBox.Yes:
            ...
            try:
                self.thcfgs.set_termination(True)
                if self.autoCalib.plot1 is not None:
                    self.autoCalib.plot1.close()
                if self.autoCalib.plot2 is not None:
...
class AutoCalibMain:
    ...
    from SensAcc.OptRefGenCheckAIO import AIOCheck
    ...
    def __init__(self, calib_window: MyMainWindow, my_settings: MySettings, thcfgs: ThreadControlFuncGenStatements):
        ...
        self.thread_aio_check = self.AIOCheck(self.thcfgs, self.start_window, self.my_settings, "127.0.0.1", 501, 1,
                                                                     100, 25, 0.003)
    ...
    def ref_check_finished(self):
        ...
        # print("END -------> REF CHECK")
        try:
            self.thread_aio_check.task_sin.close()
        except:
            pass
    ...
    def start(self):
        ...
        self.thread_aio_check = self.AIOCheck(self.thcfgs, self.start_window, self.my_settings, "127.0.0.1", 501, 1,
                                              100, 25, 0.003)
        self.thread_aio_check.check_ready.connect(self.calib_window.ref_emit)
        self.thread_aio_check.finished.connect(self.ref_check_finished)
        self.thread_aio_check.out_value_ref.connect(self.calib_window.update_ref_value)
        self.thread_aio_check.check_ready_opt.connect(self.calib_window.opt_emit)
        self.thread_aio_check.check_ready_gen.connect(self.calib_window.gen_emit)
        self.thread_aio_check.check_gen_error.connect(self.calib_window.gen_error_emit)
        self.thread_aio_check.out_value_opt.connect(self.calib_window.update_wl_values)
