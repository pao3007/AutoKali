...
print("Shape of X_train:", X_train.shape)  # Shape after train_test_split
...
print("Shape of X_train Tensor:", X_train.shape)  # Shape after conversion to tensor
...
print("Shape of X_train_unsqueezed:", X_train_unsqueezed.shape)
print("Shape of X_test_unsqueezed:", X_test_unsqueezed.shape)
...
for epoch in range(num_epochs):
    ...
    for batch_X, batch_y in train_loader:
        batch_X = batch_X.view(batch_X.size(0), input_dim, -1)
    ...
    if val_loss < best_val_loss:
        best_val_loss = val_loss
        epochs_without_improvement = 0  # Reset this count as we have a new best model
        torch.save(model.state_dict(), 'best_model.pt')
    else:
        epochs_without_improvement += 1
        print(f"No improvement for {epochs_without_improvement} epochs!")

        # If we've had 5 epochs without improvement, stop training
...
with torch.no_grad():
    X_test_unsqueezed = X_test.view(X_test.size(0), input_dim, -1)  # Reshape for Conv1d  # Add the sequence length dimension
...
optical_sensor_new, reference_sensor_new, sens = load_dataset("242352_0011", diff_folder="datasets")
...
plt.figure()
plt.plot(predicted_sensitivity_new)
plt.xlabel('predicted_sensitivity_new')
plt.ylabel('Gs')
plt.grid()
plt.show()
