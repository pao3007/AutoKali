...
from nidaqmx.constants import AcquisitionType, WAIT_INFINITELY, VoltageUnits
...
class ThreadRefSensDataCollection(QThread):
    ...
    def start_ref_sens_data_collection(self):
        ...
        self.task.ai_channels.add_ai_accel_chan(window.ref_device_name + '/' + window.ref_channel,
                                                sensitivity=window.calib_reference_sensitivity * g)

        # self.task.ai_channels.add_ai_voltage_chan(window.ref_device_name + '/' + window.ref_channel)
        ...
        data0 = self.task.read(number_of_samples_per_channel=window.ref_number_of_samples,
        ...
        g = 9.80665
        data = [(((value*1000.0) / window.calib_reference_sensitivity) / g) for value in data0]
...
class ThreadOptAndGenCheckIfReady(QThread):
    ...
    def __init__(self, server_ip, server_port, unit_id, address, number_of_samples, threshold):
        ...
        self.averager = self.RollingAverager(3)
        self.average = 0
        self.check = 0
    ...
    def check_if_ready(self):
        ...
        if self.do_action:
            ...
            print("max: " + str(max) + " | avg: " + str(self.average))
                ...
                if not window.start_sens_test:
                    ...
                    if window.opt_channels == 1:
                        ...
                        if ((not (np.any(self.samples1 == 0.0)) and int(not (np.any(self.samples2 == 0.0)))) and
                                (self.average-0.15 > max or self.average+0.15 < max)):
                            self.check += 1
                            if self.check >= 3:
                                self.check = 3
                                return 5
...
class ThreadRefCheckIfReady(QThread):
    ...
    def __init__(self, device_name_and_channel):
        ...
        self.noise = [0] * 12
        self.avg_noise = 0
        self.averager = self.RollingAverager(5)
        self.average = 0
    ...
    def run(self):
        ...
        while not self.termination:
            ...
            try:
                ...
                self.task_sin.ai_channels.add_ai_accel_chan(self.deviceName_channel,
                                                            sensitivity=window.calib_reference_sensitivity*g)
                # self.task_sin.ai_channels.add_ai_voltage_chan(self.deviceName_channel)
                ...
                # 2 varianta
                while not self.termination:
                    # print("TERM: " + str(self.termination))
                    ...
                    # print("CHECKING REF | " + str(window.start_sens_test) + "gs: " + str(max_g))
                    # print("avg: " + str(self.average) + " max: " + str(np.mean(np.abs(data))))
                    if not self.first_start:
                        if not window.start_sens_test:  # kontrola či je pripojený senzor
                            self.check_ready.emit(1)
                            timeout = 0
                            # self.average = self.averager.update(np.mean(np.abs(data)))
                        else:
...
class MyMainWindow(QMainWindow):
    ...
    def check_sensors_ready(self):
        ...
        if window.opt_channels != 0 and self.ui.S_N_line.text().strip():
            ...
            #                                  " | " + str(self.measure))
            if self.sensor_opt_check == 1 and self.sensor_ref_check == 1 and self.sensor_gen_check and not self.measure:
                self.ui.start_btn.setEnabled(True)
            else:
                self.ui.start_btn.setEnabled(False)
        else:
...
class AutoCalibMain:
    ...
    def start(self):
        window.calib_window.sensor_ref_check = 0
        window.calib_window.sensor_opt_check = 0
