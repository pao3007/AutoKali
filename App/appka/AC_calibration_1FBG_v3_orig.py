...
Scriptpath = r'C:\Users\lukac\Desktop\Sylex\AutoKali\App\appka'
...
Main_folder_path = (r"C:\Users\lukac\Desktop\Sylex\TEST_DATA\REF\test")
Opt_path = (r"C:\Users\lukac\Desktop\Sylex\TEST_DATA\REF\test\OPT")
Ref_path = (r"C:\Users\lukac\Desktop\Sylex\TEST_DATA\REF\test\REF")
...
GainMark = 150  # Frequency at which sensitivity is determined, adjust as desired
# %% Optional settings
...
Test_id = int(input(
    Fore.RED + Back.WHITE + "Select the sensor to calibrate from above and past the number here:" + Style.RESET_ALL))
# Test_id = 2
...
display(['Center wavelength = ' + str(round(np.mean(DataOptRel[0:100]), 5)) + ' nm'])
...
FreqCoherence, Coherence = signal.coherence(RefSensResized, OptSensResized, fs=BodeSampFreq, nperseg=round(NFFT / 20),
...
if Adjust_gain == 1:
    ...
    display(['Sensitivity of ' + str(sensitivities[Test_id - 1] * 1000.0) + ' pm/g' + ' at ' + str(GainMark) + ' Hz'])
else:
    display(['Sensitivity of ' + str(sensitivities[Test_id - 1] * 1000.0) + ' pm/g' + ' at ' + str(GainMark) + ' Hz'])
...
display(['Sensitivity flatness between ' + str(flatness_edge_l) + ' Hz and ' + str(flatness_edge_r) + ' Hz is: ' + str(
    round(abs(fun.interp1_for_remco(FreqTransfer, 20 * np.log(abs(SmoothTransfer)),
                                    flatness_edge_l) - fun.interp1_for_remco(FreqTransfer,
                                                                             20 * np.log(abs(SmoothTransfer)),
                                                                             flatness_edge_r)), 1))])
# %% Symmetry check
display(['Maximum acceleration = ' + str(
    round(max(opt_sens_filtered), 3)) + ' g and ' + 'minimum acceleration = ' + str(round(min(opt_sens_filtered), 4))])
display(['Difference in symmetry = ' + str(
    round(((max(opt_sens_filtered) - abs(min(opt_sens_filtered))) / max(opt_sens_filtered)) * 100, 4)) + '%'])
